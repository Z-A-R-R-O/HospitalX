...
export async function GET() {
  try {
    ...
    const [patientCount] = await db`SELECT count(*)::int AS count FROM patients`;
    const [appointmentCount] = await db`SELECT count(*)::int AS count FROM appointments WHERE starts_at >= date_trunc('day', now()) AND starts_at < date_trunc('day', now()) + interval '1 day'`;
    const [admissionCount] = await db`SELECT count(*)::int AS count FROM beds WHERE status = 'occupied'`;
    const [bedCount] = await db`SELECT count(*)::int AS count FROM beds`;
